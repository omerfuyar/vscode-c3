...
/**
 * Build the TextMate grammar JSON files from the YAML source.
 */
function buildSyntax(): void {
    let text = readFileSync('syntaxes/c3.tmLanguage.yml').toString();

    // Insert all the regular expression placeholders
    text = text.replaceAll(/\{\{([A-Za-z0-9_]+)\}\}/g, (_, key: string) => variables[key]);

    // VS Code themes are inconsistent w.r.t. supporting `entity.name` and `support.type`,
    // so we insert both in order to get consistent colors for types with all themes
    const textVscode = text
        ...
        .replaceAll(/<SCOPE_NAME_TYPE_DECL:([A-Za-z0-9_]*)>/g, (_, suffix: string) => {
            return suffix
                ? `support.type.c3 entity.name.type.${suffix}.c3`
                : 'support.type.c3 entity.name.type.c3';
        });
